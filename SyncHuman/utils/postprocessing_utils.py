...
#     Args:
#         tensor (torch.Tensor): Tensor to save (shape: [B, H, W] or [H, W])
#         filename (str): Output filename (e.g., 'output.png')
#         normalize (bool): Whether to normalize the tensor to [0, 255]
#     """
...
def export_separate_meshes_trimesh(verts, faces, inner_face_indices, outer_face_indices, 
                                 remove_face_indices, output_dir=".", output_prefix="mesh"):
    """
    导出分离的网格部分到指定文件夹
    
    参数:
        verts: 顶点数组 (torch.Tensor 或 numpy.ndarray)
        faces: 面片数组 (torch.Tensor 或 numpy.ndarray)
        inner_face_indices: 内部面片索引
        outer_face_indices: 外部面片索引
        remove_face_indices: 要移除的面片索引
        output_dir: 输出文件夹路径 (默认为当前目录)
        output_prefix: 输出文件前缀 (默认为"mesh")
    """
    # 确保输出目录存在
    os.makedirs(output_dir, exist_ok=True)
    
    # 转换为numpy数组
    verts_np = verts.cpu().numpy() if torch.is_tensor(verts) else verts
    faces_np = faces.cpu().numpy() if torch.is_tensor(faces) else faces

    inner_face_indices = inner_face_indices.cpu().numpy() if torch.is_tensor(inner_face_indices) else inner_face_indices
    outer_face_indices = outer_face_indices.cpu().numpy() if torch.is_tensor(outer_face_indices) else outer_face_indices
    remove_face_indices = remove_face_indices.cpu().numpy() if torch.is_tensor(remove_face_indices) else remove_face_indices
    
    # 创建剩余面片的掩码（既不是inner也不是outer）
    mask = np.ones(len(faces_np), dtype=bool)
    mask[inner_face_indices] = False
    mask[outer_face_indices] = False
    remaining_face_indices = np.where(mask)[0]

    # 导出inner_faces部分
    if len(inner_face_indices) > 0:
        inner_mesh = trimesh.Trimesh(
            vertices=verts_np,
            faces=faces_np[inner_face_indices],
            process=False
        )
        output_path = os.path.join(output_dir, f"{output_prefix}_inner.ply")
        inner_mesh.export(output_path)
        print(f"Exported inner mesh: {output_path} (faces={len(inner_face_indices)})")
    else:
        print("No inner faces to export")

    # 导出remove_faces部分
    if len(remove_face_indices) > 0:
        remove_mesh = trimesh.Trimesh(
            vertices=verts_np,
            faces=faces_np[remove_face_indices],
            process=False
        )
        output_path = os.path.join(output_dir, f"{output_prefix}_remove.ply")
        remove_mesh.export(output_path)
        print(f"Exported remove mesh: {output_path} (faces={len(remove_face_indices)})")
    else:
        print("No remove faces to export")

    # 导出outer_faces部分
    if len(outer_face_indices) > 0:
        outer_mesh = trimesh.Trimesh(
            vertices=verts_np,
            faces=faces_np[outer_face_indices],
            process=False
        )
        output_path = os.path.join(output_dir, f"{output_prefix}_outer.ply")
        outer_mesh.export(output_path)
        print(f"Exported outer mesh: {output_path} (faces={len(outer_face_indices)})")
    else:
        print("No outer faces to export")

    # 导出剩余部分
    if len(remaining_face_indices) > 0:
        remaining_mesh = trimesh.Trimesh(
            vertices=verts_np,
            faces=faces_np[remaining_face_indices],
            process=False
        )
        output_path = os.path.join(output_dir, f"{output_prefix}_remaining.ply")
        remaining_mesh.export(output_path)
        print(f"Exported remaining mesh: {output_path} (faces={len(remaining_face_indices)})")
    else:
        print("No remaining faces to export")
...
def _fill_holes(
    ...
    resolution=128,
    num_views=500,
    min_outer_hole_ratio=0.7,
    min_outer_hole_min=0.15,
    max_inner_hole=0.01,
    debug=False,
    verbose=False
):
    ...
    for i in tqdm(range(views.shape[0]), total=views.shape[0], disable=not verbose, desc='Rasterizing'):
        ...
        # save_tensor_as_image(buffers['mask'], 'mask.png')
        # depth = 1.0 - buffers['depth']  # 反转深度值，使 near=white, far=black
        # save_tensor_as_image(depth, 'depth.png')
    ...
    for i in range(len(connected_components)):
        outer_face_indices[connected_components[i]] = visblity[connected_components[i]] > min(max(min(visblity[connected_components[i]].quantile(min_outer_hole_ratio).item(),visblity.quantile(min_outer_hole_ratio).item()), min_outer_hole_min), 0.5)

    outer_face_indices = outer_face_indices.nonzero().reshape(-1)
    
    ## construct inner faces
    inner_face_indices = torch.nonzero(visblity <=max_inner_hole).reshape(-1)
    if verbose:
    ...
    # import random
    # import string
    # output_prefix = ''.join(random.choices(string.ascii_lowercase + string.digits, k=8))
    # export_separate_meshes_trimesh(verts, faces, inner_face_indices, outer_face_indices, remove_face_indices, 
    #                          output_dir="./outputsee", output_prefix=output_prefix )
    ...
    for cc in to_remove_cc:
        ...
        if visblity_median > min_outer_hole_min:
            continue
...
def postprocess_mesh(
    ...
    if fill_holes:
        ...
        hole_filling_params = [
            {"min_outer_hole_ratio": 0.7, "min_outer_hole_min": 0.15, "max_inner_hole": 0.01},
            {"min_outer_hole_ratio": 0.7, "min_outer_hole_min": 0.15, "max_inner_hole": 0},
            {"min_outer_hole_ratio": 0.6, "min_outer_hole_min": 0.08, "max_inner_hole": 0.01},
        ]

        # 初始调用
        vertices_after, faces_after, num_removed = _fill_holes(
            vertices, faces,
            ...
            debug=debug,
            verbose=verbose,
            **hole_filling_params[0]
        )

        for params in hole_filling_params[1:]:
            
            v_num=vertices.shape[0]
            print("num_removed:",num_removed)
            print("v_num=:",v_num)
            if num_removed > 15000 or num_removed > v_num*(4/5):
                break
            vertices_after, faces_after, num_removed = _fill_holes(
                vertices, faces,
                max_hole_size=fill_holes_max_hole_size,
        ...
        vertices, faces = vertices_after.cpu().numpy(), faces_after.cpu().numpy()
    if verbose:
...
def bake_texture(
    ...
    projection_ortho=get_ortho_projection_matrix(-0.5, 0.5, -0.5, 0.5, 0.1, 100)
    projections =[torch.tensor(projection_ortho).cuda()]*5+ [utils3d.torch.intrinsics_to_perspective(torch.tensor(intr).cuda(), near, far) for intr in intrinsics[5:]]
    ...
    if mode == 'fast':
        ...
        rastctx = utils3d.torch.RastContext(backend='cuda')
        observations =observations[:5]+ [observation.flip(0) for observation in observations[5:]]
        masks = masks[:5]+ [m.flip(0) for m in masks[5:]]
        _uv = []
        _uv_dr = []
        ...
        with tqdm(total=total_steps, disable=not verbose, desc='Texture baking (opt): optimizing') as pbar:
            for step in range(total_steps):
                optimizer.zero_grad()
                # selected = np.random.randint(5, len(views))
                if step<=930:
                    selected = np.random.randint(0, len(views))
                else:
                    selected = np.random.choice([0,2])
                # elif step<=950:
                #     selected = np.random.choice([0,2,4])
                # else:
                #     selected = 4
                ...
                optimizer.step()
                # annealing
                optimizer.param_groups[0]['lr'] = cosine_anealing(optimizer, step, total_steps, 1e-2, 3e-5)
...
def to_glb(
    ...
    masks=[]
    for i in range(4):
        observations[i]=(mv_img[i].permute(1, 2, 0).numpy()* 255).astype(np.uint8)
        masks.append(np.any((mv_normal[i].permute(1, 2, 0).numpy()*255) < 230, axis=-1))
        if i in [2,3]:
            observations[i]=observations[i][:,::-1,:]
            masks[i]=masks[i][:,::-1]
 
    observations[4]= (np.transpose(faces_img, (1, 2, 0)) * 255).astype(np.uint8)
    masks.append(np.any((np.transpose(faces_normal, (1, 2, 0))* 255) < 230, axis=-1))


    masks = masks+[np.any(observation > 0, axis=-1) for observation in observations[5:]]
